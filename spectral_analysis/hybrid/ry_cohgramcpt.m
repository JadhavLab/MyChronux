...
for n=1:nw;
   ...
   t=indx/Fs;
   datawin1=data1(indx,:);
   datawin2=extractdatapt(data2,[indx(1)/Fs indx(end)/Fs]);
   if nargout==11;
...
function initialize(output_level)
  ...
  if isstruct(c)
      for fd = fieldnames(c)'; fd=fd{1};
       ...
       elseif numel(fd)>2 && all(fd(1:2)=='du')
        ...
        C.(fd)=single(zeros(nw,Nf,1, kwargs{:}));
       end
      end
  else
     C=single(zeros(nw,Nf, kwargs{:}));
  end
